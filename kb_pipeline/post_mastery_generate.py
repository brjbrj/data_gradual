...
def _build_prompt(
    ...
    user_lines = [
        ...
        "- Use connective reasoning words such as First, Next, Then, After that, So, Therefore, or Finally to make the dependency structure clear.",
        "- Each step must make its source clear: it may come directly from a condition in the problem, from one previous intermediate result, or from combining two or more previously found quantities.",
        "- Do not force every step to depend only on the immediately previous step. Some correct solutions first compute independent quantities and later combine them.",
        "- For every equation, briefly state why that equation is relevant and what quantity it produces for the solution.",
        "- Make the reasoning flow similar to GSM8K solutions: 'From the problem, ... so ...', 'Since we know ..., ...', 'Combining these quantities, ...', 'This means ...', 'Therefore ...'.",
        "- Do not write trial-and-error, alternative methods, or unnecessary reflection. The chain should be the direct correct solution path only.",
        "- Avoid a bare computation list. A reader should understand why every formula appears and how its result is used later.",
        "- If a step produces a value needed later, explicitly name that value, such as total needed per day, amount covered, remaining amount, combined cost, or final profit.",
